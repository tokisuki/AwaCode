...
功能：Qt 控制台和命令行都通过 JSON-RPC 管理 TypeScript Core。Core 自行实现会话 SQLite 持久化、流式模型调用、Plan→Execute→Reflect 循环、上下文滚动摘要、全局/项目显式记忆、崩溃恢复，以及列举、读取、搜索、精确编辑、新建文件和命令等本地工具。读操作自动允许；写文件和命令每次都需 allow_once 或 deny。
